...
func (g *GitHubRunner) Uninstall() error {
	// 运行卸载脚本
	g.SoftwareLogger.Infof("正在卸载 GitHub Runner...")
	...
	g.SoftwareLogger.Success("GitHub Runner 卸载完成")

	return nil
}

func (g *GitHubRunner) GetStatus() (map[string]string, error) {
	g.SoftwareLogger.Infof("获取 GitHub Runner 状态...")
	status := "not_installed"

	// 检查安装目录是否存在
	if _, err := os.Stat(g.targetDir + "/run.sh"); !os.IsNotExist(err) {
		g.SoftwareLogger.Infof("GitHub Runner 目录存在")
		status = "stopped"
	...
	g.SoftwareLogger.Infof("GitHub Runner 状态: %s", status)
...
func (g *GitHubRunner) Start() error {
	...
	if os.Geteuid() != 0 {
		...
		return g.SoftwareLogger.LogAndReturnErrorf("%s", err.Error())
	}
	...
	if _, err := os.Stat(g.targetDir + "/.runner"); os.IsNotExist(err) {
		g.SoftwareLogger.Infof("Runner 未配置，请输入以下信息：")

		var url, token string
		g.SoftwareLogger.Infof("请输入 GitHub 仓库/组织 URL：")
		fmt.Scanln(&url)

		g.SoftwareLogger.Infof("请输入 Runner 注册令牌：")
		fmt.Scanln(&token)
		...
		if err != nil {
			return g.SoftwareLogger.LogAndReturnErrorf("配置 runner 失败: %s", err.Error())
		}
	}
	...
	g.SoftwareLogger.Success("GitHub Runner 已启动")
	return nil
}
