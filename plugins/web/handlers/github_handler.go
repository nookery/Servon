...
func (h *WebHandler) HandleGitHubSetup(c *gin.Context) {
	manifest, err := h.App.GitHubManager.HandleSetup(c)
	if err != nil {
...
func (h *WebHandler) HandleGitHubCallback(c *gin.Context) {
	...
	redirectURL, err := h.App.GitHubManager.HandleCallback(c)
	if err != nil {
...
func (h *WebHandler) HandleGitHubWebhook(c *gin.Context) {
	...
	if err := h.App.GitHubManager.HandleWebhook(c); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *WebHandler) HandleGetWebhooks(c *gin.Context) {
	...
	webhooks, err := h.App.GitHubManager.GetStoredWebhooks()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
