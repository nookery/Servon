...
func (w *WebRouter) SetupUIRoutes(router *gin.Engine, isDev bool) {
	if isDev {
		go func() {
			w.runNpmDev(9754)
		}()
	...
	w.setupStaticRoutes(router)
}

func (w *WebRouter) setupStaticRoutes(router *gin.Engine) {
	// 获取嵌入的dist子文件系统
...
func (w *WebRouter) runNpmDev(backendPort int) {
	w.Handler.App.Printer.PrintInfof("开发环境，启动 npm dev server")
	...
	err := cmd.Run()
	if err != nil {
		w.Handler.App.Printer.PrintErrorf("启动 npm dev server 失败: %v", err)
	}
}
