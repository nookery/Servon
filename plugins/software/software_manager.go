...
var (
	// registry 存储所有已注册的软件
	registry = make(map[string]func() contract.SuperSoft)
	regMutex sync.RWMutex
)

// RegisterSoftware 注册一个新的软件到注册表中
func RegisterSoftware(name string, factory func() contract.SuperSoft) {
	regMutex.Lock()
...
func NewSoftware(name string) (contract.SuperSoft, error) {
	regMutex.RLock()
