...
download_latest() {
    ...
    if [ -z "$LATEST_VERSION" ]; then
        print_error "Failed to determine the latest version. Please check the error messages above."
        exit 1
    fi
    ...
    # 更新下载URL路径
    DOWNLOAD_URL="https://github.com/nookery/Servon/releases/download/${LATEST_VERSION}/servon-${OS}-${ARCH}"
    CHECKSUM_URL="https://github.com/nookery/Servon/releases/download/${LATEST_VERSION}/servon-${OS}-${ARCH}.sha256"
    
    print_info "Downloading Servon ${LATEST_VERSION} for ${OS}/${ARCH}..."

    # 下载二进制文件
    print_info "Downloading binary..."
    run_command curl -L -o "/tmp/servon" "$DOWNLOAD_URL"

    # 下载校验和文件
    print_info "Downloading checksum..."
    run_command curl -L -o "/tmp/servon.sha256" "$CHECKSUM_URL"
